...
class SpectrogramDiscriminator(torch.nn.Module):
    def __init__(self, spec_transform, fc2_input_size=360):
        super(SpectrogramDiscriminator, self).__init__()
        self.spec_transform = spec_transform
        self.conv1 = torch.nn.Conv2d(1, 10, kernel_size=(3, 3), stride=(2, 2), padding=1)
        self.conv2 = torch.nn.Conv2d(10, 20, kernel_size=(3, 3), stride=(2, 2), padding=1)
        self.conv3 = torch.nn.Conv2d(20, 20, kernel_size=(3, 3), stride=(2, 2), padding=1)
        self.conv4 = torch.nn.Conv2d(20, 20, kernel_size=(3, 3), stride=(2, 2), padding=1)
        self.conv5 = torch.nn.Conv2d(20, 30, kernel_size=(3, 3), stride=(2, 2), padding=1)
    ...
    def forward(self, x):
        ...
        if self.fc1 is None:
            self.fc1 = torch.nn.Linear(x.shape[1], self.fc2_input_size)
            self.fc2 = torch.nn.Linear(self.fc2_input_size, 1)
