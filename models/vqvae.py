...
from models.layers import Decoder, Encoder, VectorQuantizer, SoundStreamEncoder, SoundStreamDecoder, SpectrogramDiscriminator
...
class VQVAE(LightningModule):  # TODO this perhaps could inherit after AE...? There's quite a lot of duplicated code...
    def __init__(self, args_dict):
        ...
        self.spectrogram_discriminator = SpectrogramDiscriminator(self.mel_transform)

        # loss function
        self.reconstruction_loss_fn = torch.nn.MSELoss(reduction='mean')
        self.dicrimination_loss_fn = torch.nn.BCELoss()
    ...
    def forward(self, x):
        ...
        raw_spec_jugment = self.spectrogram_discriminator(x)
        procceded_spec_jugment = self.spectrogram_discriminator(x_hat)
        return x_hat, embedding_loss, perplexity, raw_spec_jugment, procceded_spec_jugment

    def training_step(self, batch, batch_idx):
        x_hat, embedding_loss, perplexity, raw_spec_disc_jugment, proceded_spec_disc_jugnment = self(batch)
        
        recon_loss = self.reconstruction_loss_fn(x_hat, batch)
        dicriminator_loss_raw = self.dicrimination_loss_fn(raw_spec_disc_jugment, torch.ones_like(raw_spec_disc_jugment))
        dicriminator_loss_procceded = self.dicrimination_loss_fn(proceded_spec_disc_jugnment, torch.zeros_like(raw_spec_disc_jugment))
        self.log_training_step_metrics(recon_loss, embedding_loss, perplexity, x_hat, batch, batch_idx, dicriminator_loss_procceded, dicriminator_loss_raw)

        return recon_loss + embedding_loss + dicriminator_loss_procceded/2 + dicriminator_loss_raw/2
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_validation_step_metrics(recon_loss, embedding_loss, perplexity, x_hat, batch, batch_idx, dicriminator_loss_procceded, dicriminator_loss_raw)
    ...
    @torch.no_grad()
    def log_training_step_metrics(self, recon_loss, embedding_loss, perplexity, x_hat, batch, batch_idx, raw_discrimination, procceded_discrimination):
        # log losses
        ...
        self.log('procceded_discrimination', procceded_discrimination, sync_dist=True, batch_size=self.batch_size)
        self.log('raw_discrimination', raw_discrimination, sync_dist=True, batch_size=self.batch_size)
    ...
    def log_validation_step_metrics(self, recon_loss, embedding_loss, perplexity, x_hat, batch, batch_idx, raw_discrimination, procceded_discrimination):
        # log losses
